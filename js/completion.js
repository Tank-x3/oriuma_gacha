...
const completionManager = {
    ...
    checkNewCharacters() {
        if (!this.data) return;
        const normalHasNew = this._checkNewCharsForScope('normalComplete', this._getNormalTargetIds());
        const fullHasNew = this._checkNewCharsForScope('fullComplete', this._getFullTargetIds());

        // 未達スコープで新キャラが検出された場合、info バナーに1回だけ通知
        if (normalHasNew || fullHasNew) {
    ...
    _checkNewCharsForScope(scope, currentAllIds) {
        ...
        // 新キャラ検出: 追加分モードでは knownAllIds（差分追跡開始時の全ID）をベースラインにする
        const baseline = (prog.mode === '追加分' && prog.knownAllIds) ? prog.knownAllIds : prog.targetIds;
        const newIds = currentAllIds.filter(id => !baseline.includes(id));
        ...
        if (isCompleted) {
            ...
            if (window.toastManager) {
                window.toastManager.showPersistent(
                    ...
                    [
                        {
                            label: 'A) 差分追跡',
                            onClick: () => this._applyDiffTracking(scope, newIds, currentAllIds)
                        },
                        {
                            label: 'B) 全リセット',
            ...
            // 未達 → targetIds をモードに応じて更新
            if (prog.mode === '追加分') {
                // 追加分モード: 新キャラIDのみを追加（既存の追跡対象を維持）
                newIds.forEach(id => {
                    if (!prog.targetIds.includes(id)) {
                        prog.targetIds.push(id);
                    }
                });
                if (!prog.newCharacterIds) prog.newCharacterIds = [];
                newIds.forEach(id => {
                    if (!prog.newCharacterIds.includes(id)) {
                        prog.newCharacterIds.push(id);
                    }
                });
                // 次回検出に備えて既知IDリストを更新
                prog.knownAllIds = currentAllIds;
            } else {
                // initial / 全リセット: targetIds を全IDに更新（現行動作）
    ...
    _applyDiffTracking(scope, newIds, allIds) {
        const prog = this.data.currentProgress[scope];
        const scopeStr = scope === 'normalComplete' ? 'normal' : 'full';
